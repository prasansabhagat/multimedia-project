...
def open_encode_file1(frame):
    e_pg = Frame(root)
    global my_file1
    my_file1 = fd.askopenfilename(filetypes=([('jpg', '.jpg'),('png', '.png'), ('jpeg', '.jpeg'), ('All Files', '.*')]))
    if not my_file1:
        tk.messagebox.showerror("Error", "You have selected nothing !")
    else:
        global my_img1
        print(my_file1)
        my_img1 = Image.open(my_file1)
        new_image = my_img1.resize((300, 200))
        img = ImageTk.PhotoImage(new_image)
        label3 = Label(frame, text='Selected Carrier Image')
        label3.config(font=('Helvetica', 14, 'bold'))
        label3.grid(row=5, column = 0)
        board = Label(frame, image=img)
        board.image = img
...
def open_encode_file2(frame):
    e_pg = Frame(root)
    global my_file2
    my_file2 = fd.askopenfilename(filetypes=([('jpg', '.jpg'),('png', '.png'), ('jpeg', '.jpeg'), ('All Files', '.*')]))
    if not my_file2:
        tk.messagebox.showerror("Error", "You have selected nothing !")
    else:
        global my_img2
        print(my_file2)
        my_img2 = Image.open(my_file2)
        new_image = my_img2.resize((300, 200))
        img = ImageTk.PhotoImage(new_image)
        label3 = Label(frame, text='Selected Secret Image')
        label3.config(font=('Helvetica', 14, 'bold'))
        label3.grid(row=7, column = 0)
        board = Label(frame, image=img)
        board.image = img
        board.grid(row=8, column = 0)
        root.geometry("700x850")
        print("image loaded")
        
def open_decode_file1(frame):
    d_F3 = Frame(root)
    my_file = fd.askopenfilename(filetypes=([('png', '.png'), ('jpeg', '.jpeg'), ('jpg', '.jpg'), ('All Files', '.*')]))
    if not my_file:
        tk.messagebox.showerror("Error", "You have selected nothing !")
        ...
        os.system(f'python trial.py unmerge --image={my_file} --output={op_path}')
        
        new_img=Image.open(op_path)
        new_img=new_img.resize((300,200))
        img_new=ImageTk.PhotoImage(new_img)
        label5= Label(frame,text='Recovered Image :')
        label5.config(font=('Helvetica',14,'bold'))
        label5.grid()
        board1 = Label(frame, image=img_new)
        board1.image = img_new
        board1.grid()
        # hidden_data = decode(my_img_decode)                #to be decrypted (hidden data)
        # hidden_data=obj.decrypt(hidden_data)
        # label2 = Label(frame, text='Hidden data is :')
...
def encode_img_func(img1,img2):
    img_=Image.open(img1)
    temp=os.path.splitext(os.path.basename(img_.filename))[0]
    op_path=tk.filedialog.asksaveasfilename(initialfile=temp,filetypes = ([('png', '*.png')]),defaultextension=".png")
...
def encode_image(frame):
    ...
    frame = LabelFrame(root, text="Image in Image", padx=20, pady = 10)
    frame.pack()
    root.geometry("500x450")

    select_image = Label(frame, text="Select the Image in which you want to hide another image:")
    select_image.grid(row=0)

    select_button = Button(frame, text = "Select", command=lambda:open_encode_file1(frame))
    select_button.grid(row=1, column=0, pady=10)
    
    select_image = Label(frame, text="Select the Image which you want to hide:")
    select_image.grid(row=2)

    select_button1 = Button(frame, text = "Select", command=lambda:open_encode_file2(frame))
    select_button1.grid(row=3, column=0, pady=10)

    hide_button = Button(frame, text="Hide",command=lambda: encode_img_func(my_file1 ,my_file2) )
    hide_button.grid(row=4, column = 0)
...
def decode_image(frame):
    ...
    select_encoded_image = Label(frame, text="Select the image with hidden image:")
    select_encoded_image.grid(row = 1, column = 0, pady = 10)

    select_button = Button(frame, text="Select", command = lambda:open_decode_file1(frame))
    select_button.grid(row=2, column=0, pady=10)

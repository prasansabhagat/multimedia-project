...
def TextInImage(frame):
    ...
    encode_text_button = Button(
        frame, text="Encode", command=lambda: encode_text(frame))
    encode_text_button.grid(row=1, column=0, pady=20)

    decode_text_button = Button(
        frame, text="Decode", command=lambda: decode_text(frame))
...
def home(frame):
    ...
    option1_button = ttk.Button(
        frame, text="Text in Image", command=lambda: TextInImage(frame))
    option1_button.pack(fill="x", padx="30", pady="10")

    option2_button = ttk.Button(
        frame, text="Image in Image", command=lambda: ImageInImage(frame))
...
def encode_text(frame):
    ...
    frame.pack()
    select_image = Label(
        frame, text="Select the Image in which you want to hide text:")
...
def decode_text(frame):
    ...
    select_encoded_image = Label(
        frame, text="Select the image with hidden text:")
